(function(root, factory) {
    ...
    Parser.prototype.processNodeHeader = function() {
        ...
        ['content-type', 'content-disposition'].forEach(function(key) {
            if (_self._node.parsedHeader[key]) {
                _self._node.parsedHeader[key] = _self.parseValueParams([].concat(_self._node.parsedHeader[key] || []).pop());
            }
        });
        ...
        ['from', 'sender', 'reply-to', 'to', 'cc', 'bcc'].forEach(function(key) {
            var addresses = [];
            if (_self._node.parsedHeader[key]) {
                [].concat(_self._node.parsedHeader[key] || []).forEach(function(value) {
                    if (value) {
                        addresses = addresses.concat(addressparser.parse(value) || []);
                    }
                });
                _self._node.parsedHeader[key] = addresses;
            }
        });
    };
